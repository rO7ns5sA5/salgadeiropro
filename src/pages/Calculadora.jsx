...
function CurrencyInput({ label, value, onChange }) {
  return (
    <div>
      ...
      >
        <span className="text-sm" style={{ color: '#9CA3AF' }}>R$</span>
        <input
          type="number"
          min={0}
          step={0.1}
          ...
          style={{ color: '#1A1A1A' }}
        />
        <span className="text-xs" style={{ color: '#9CA3AF' }}>/kg</span>
      </div>
    </div>
  )
}
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-2">
        <label className="text-sm font-medium" style={{ color: '#666666' }}>{label}</label>
        <div
          className="px-3 py-0.5 rounded-lg text-sm font-bold"
          style={{ backgroundColor: GOLD, color: '#FFFFFF', minWidth: 44, textAlign: 'center' }}
        >
          {value}%
        </div>
      </div>
      <input
        ...
        style={{ accentColor: GOLD, backgroundColor: '#E5E5E5' }}
      />
    </div>
  )
}
  ...
  return (
    <PageLayout titulo="Produção">
      <div className="px-4 pt-5 pb-6 space-y-5">
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              style={{ backgroundColor: '#FFFFFF', border: '1px solid #E5E5E5', color: '#666666' }}
            >
              −
            </button>
            <div
              className="flex-1 flex items-center justify-center rounded-xl h-11"
            ...
            <button
              ...
              className="w-11 h-11 rounded-xl flex items-center justify-center font-bold text-lg flex-shrink-0"
              style={{ backgroundColor: GOLD, color: '#FFFFFF', border: 'none' }}
            >
              +
            </button>
          </div>
        ...
        >
          <p className="font-semibold text-sm" style={{ color: '#1A1A1A' }}>Proporções</p>
          <SliderField
            label="Proporção Massa (%)"
            value={form.proporcaoMassa}
            onChange={(v) => { set('proporcaoMassa')(v); set('proporcaoRecheio')(100 - v) }}
          />
          <SliderField
            label="Proporção Recheio (%)"
            value={form.proporcaoRecheio}
            onChange={(v) => { set('proporcaoRecheio')(v); set('proporcaoMassa')(100 - v) }}
          />
        </div>
        ...
        <div
          ...
          style={{ backgroundColor: '#FFFFFF', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', border: '1px solid #F0F0F0' }}
        >
          <p className="font-semibold text-sm" style={{ color: '#1A1A1A' }}>Custos</p>
